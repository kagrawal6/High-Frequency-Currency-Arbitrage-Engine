...
static Options parseArgs(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--verbose") opt.verbose = true;
            ...
            std::cout << "Usage: ./arbitrage [--max-ts N] [--block B] [--verbose] [--no-trade]\n";
            std::exit(0);
        }
    }
    return opt;
...
int main(int argc, char** argv) {
    ...
    std::cout << "========== FOREX ARBITRAGE DETECTOR (CUDA) ==========\n\n";
    std::cout << "Backend: " << cudaBackendName() << "  block=" << opt.blockSize << "\n";
    std::cout << "Currencies:";
    ...
    detector.analyzeAllTimestamps(true);
